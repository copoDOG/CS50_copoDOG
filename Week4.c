#include <stdio.h>
#define MAX 5 //MAX를 5로 정의한다.
#define TRUE 1 //1을 true로 정의한다.
#define FALSE 0 //0을 false로 정의한다.

void selectSort(int *arr); //선택정렬
void bubbleSort(int *arr); //버블정렬
void mergeSort(int *arr, int start, int end); //합병정렬
void swap(int x, int y, int tmp, int *arr);
void compare(int *x, int *y);
void merge(int *arr, int start, int mid, int end);

int selectSort_1[MAX] = {1, 2, 3, 4, 5}; // 예)에 제시되어있는 입력값을 배열에 저장
